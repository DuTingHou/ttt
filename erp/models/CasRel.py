...
class CasRel(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.bert = BertModel.from_pretrained(self.config.get("bert_name"))
        self.sub_heads_linear = nn.Linear(self.config.get("bert_dim"), 1)
        self.sub_tails_linear = nn.Linear(self.config.get("bert_dim"), 1)
        self.obj_heads_linear = nn.Linear(self.config.get("bert_dim"), self.config.get("num_relations"))
        self.obj_tails_linear = nn.Linear(self.config.get("bert_dim"), self.config.get("num_relations"))
    ...
    def forward(self, token_ids, mask, sub_head, sub_tail):
        ...
        return pred_sub_heads.squeeze(-1) ,pred_sub_tails.squeeze(-1) ,pred_obj_heads.squeeze(-1) ,pre_obj_tails.squeeze(-1)
        return {
